    ...
    public partial class SettingsForm : Form
    {
        ...
        private void MultiRobloxCB_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (!AccountManager.Instance.UpdateMultiRoblox())
                MessageBox.Show("Roblox сейчас запущен. Multi Roblox не заработает, пока Roblox открыт.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        ...
        {
            if (Utilities.YesNoPrompt("Настройки", "Change Encryption Method", "Are you sure you want to change how your data is encrypted?", false))
        ...
        private void EnableWSCB_CheckedChanged(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Чтобы применить эту настройку, нужно перезапустить akso alt", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void AllowExternalConnectionsCB_CheckedChanged(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Чтобы применить эту настройку, нужно перезапустить akso alt\n\nДля этой настройки нужны права администратора", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
            ...
            if (OverrideWithCustomCB.Checked)
            {
                if (CustomClientSettingsDialog.ShowDialog() == DialogResult.OK)
                {
                    if (File.Exists(CustomClientSettingsDialog.FileName) && File.ReadAllText(CustomClientSettingsDialog.FileName).TryParseJson<object>(out _))
                    {
                        ...
                        MessageBox.Show("Выбран неверный файл. Убедись, что внутри корректный JSON", "Настройки", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                    Remove();
            }
