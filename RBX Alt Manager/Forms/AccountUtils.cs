...
namespace RBX_Alt_Manager
{
    public partial class AccountUtils : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show($"Точно выйти из всех других сессий?", "Утилиты аккаунта", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void Block_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (AccountManager.SelectedAccount.TogglePlayerBlocked(Username.Text, ref WasUnblocked))
                    MessageBox.Show($"{(WasUnblocked ? "Разблокирован" : "Заблокирован")} пользователь {Username.Text}");
                else
                    MessageBox.Show($"Не удалось {(WasUnblocked ? "разблокировать" : "заблокировать")} {Username.Text}");
            }
            catch (Exception x) { MessageBox.Show($"Не удалось {(WasUnblocked ? "разблокировать" : "заблокировать")} {Username.Text}\n\n{x.Message}"); }
        }
        ...
        private void SetDisplayName_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                MessageBox.Show($"Display Name аккаунта {AccountManager.SelectedAccount.Username} изменён на {DisplayName.Text}", "Утилиты аккаунта", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception x) { MessageBox.Show(x.Message, "Утилиты аккаунта", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
