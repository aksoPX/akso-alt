...
namespace RBX_Alt_Manager.Forms
{
    public partial class AccountControl : Form
    {
        ...
        private void AccountControl_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"{x.Message} {x.StackTrace}", "Управление аккаунтами", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Hide();
            }
        ...
        private void AutoExecuteScriptBox_Leave(object sender, EventArgs e)
        {
            ...
            if (AccountsView.SelectedObjects.Count == 1 || (AccountsView.SelectedObjects.Count > 1 && MessageBox.Show($"Применить эти изменения к аккаунтам? Количество: {AccountsView.SelectedObjects.Count}", "Предупреждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
            {
                foreach (ControlledAccount account in AccountsView.SelectedObjects)
        ...
        {
            if (MessageBox.Show("Ты уверен?", "Управление аккаунтами", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
