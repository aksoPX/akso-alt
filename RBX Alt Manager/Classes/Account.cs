...
namespace RBX_Alt_Manager
{
    public class Account : IComparable<Account>
    {
        ...
        public bool CheckPin(bool Internal = false)
        {
            ...
            {
                if (!Internal) MessageBox.Show("Невалидная сессия аккаунта!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (!Internal) MessageBox.Show("Требуется PIN!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public bool UnlockPin(string Pin)
        {
            ...
            if (response.IsSuccessful && response.StatusCode == HttpStatusCode.OK)
            {
                ...
                {
                    MessageBox.Show("PIN разблокирован на 5 минут", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        public bool ChangePassword(string Current, string New)
        {
            ...
            if (response.IsSuccessful && response.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (SToken != null)
                {
                    ...
                    MessageBox.Show("Ошибка при смене пароля. Нужно заново войти с новым паролем!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);

                MessageBox.Show("Пароль изменён!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);

                return true;
            }

            MessageBox.Show("Не удалось сменить пароль!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public bool ChangeEmail(string Password, string NewEmail)
        {
            ...
            if (response.IsSuccessful && response.StatusCode == HttpStatusCode.OK)
            {
                MessageBox.Show("Email изменён!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);

                return true;
            }

            MessageBox.Show("Не удалось сменить email. Возможно, пароль неверный!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public bool LogOutOfOtherSessions(bool Internal = false)
        {
            ...
            if (response.IsSuccessful && response.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (SToken != null)
                {
                    ...
                    MessageBox.Show("Произошла ошибка, нужно войти заново!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);

                if (!Internal) MessageBox.Show("Выполнен выход из всех других сессий!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);

                return true;
            }

            if (!Internal) MessageBox.Show("Не удалось выйти из других сессий!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (GetAuthTicket(out string Ticket))
            {
                ...
                if (JoinVIP)
                {
                    ...
                    if (result.IsSuccessful && !result.Content.Contains("\"AllUsers\""))
                    {
                        ...
                        {
                            if (Utilities.YesNoPrompt(RussianLocalization.AppName, "Настройки приватности аккаунта не позволяют входить на приватные серверы.", "Изменить настройку на Everyone сейчас?"))
                            {
