    ...
    static class Program
    {
        ...
        static void Main(params string[] Arguments)
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("akso alt нужно распаковать в отдельную папку, иначе программа может работать некорректно!", RussianLocalization.AppName, MessageBoxButtons.OK);
            ...
            if (!IsAutoUpdater && !File.Exists($"{Application.ExecutablePath}.config"))
            {
                ...
                {
                    if (!Utilities.YesNoPrompt(RussianLocalization.AppName, "Рекомендуется устанавливать akso alt в отдельную папку", "Пропустить проверку и использовать эту папку?", false))
                        Environment.Exit(4);
                }
            }
            ...
            using (var ndpKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32)?.OpenSubKey(subkey))
            {
                ...
                {
                    if (MessageBox.Show("Не найден .NET Framework 4.7.2. Установить его сейчас?\nОн нужен для работы akso alt.\nЕсли ошибка повторяется — полностью переустанови программу.", RussianLocalization.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            ...
            if (mutex.WaitOne(TimeSpan.Zero, true))
            {
                ...
                MessageBox.Show("akso alt уже запущен!", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
#endif
        }
    }
}