...
namespace RBX_Alt_Manager
{
    public partial class AccountManager : Form
    {
        ...
        private Label AccountStatsLabel;
        private Microsoft.Web.WebView2.WinForms.WebView2 AksoWebView;
        private long AksoSelectedUserId = -1;
        ...
        public AccountManager()
        {
            ...
            if (!General.Exists("WindowScale"))
            {
                ...
                if (Program.Scale > 1)
                    if (!Utilities.YesNoPrompt(RussianLocalization.AppName, "Обнаружен монитор больше среднего", $"Оставить масштаб окна {Program.Scale:F2}?", false))
                        General.Set("WindowScale", "1.0");
                    else
                        MessageBox.Show("Если масштаб шрифта отображается неправильно, открой RAMSettings.ini и поменяй \"ScaleFonts=true\" на \"ScaleFonts=false\" без кавычек.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            ...
            Icon = ThemeEditor.UseDarkTopBar ? Properties.Resources.team_KX4_icon_white : Properties.Resources.team_KX4_icon; // akso alt icon
        ...
        private void CenterAksoForm(Form form)
        {
            if (form == null) return;
            ...
            form.Left = screen.Left + Math.Max(0, (screen.Width - form.Width) / 2);
            form.Top = screen.Top + Math.Max(0, (screen.Height - form.Height) / 2);
        ...
        private void AddAksoMenuItems()
        {
            try
            {
                if (infoToolStripMenuItem != null)
                {
                    
                    infoToolStripMenuItem.DropDownItems.Insert(0, new ToolStripMenuItem("Проверка безопасности", null, (s, e) => ShowSecurityAudit()));
                    infoToolStripMenuItem.DropDownItems.Add(new ToolStripSeparator());
                    infoToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("Открыть папку программы", null, (s, e) => Process.Start(Environment.CurrentDirectory)));
                    infoToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("Сделать backup AccountData", null, (s, e) => CreateAccountDataBackup()));
                    infoToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("Экспорт настроек без аккаунтов", null, (s, e) => ExportPublicSettings()));
                }

                if (copyToolStripMenuItem != null)
                {
                    copyToolStripMenuItem.DropDownItems.Add(new ToolStripSeparator());
                    copyToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("Очистить буфер обмена", null, (s, e) =>
                    {
                        Clipboard.Clear();
                        MessageBox.Show("Буфер обмена очищен.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }));
                    copyToolStripMenuItem.DropDownItems.Add(new ToolStripMenuItem("Очистить буфер через 30 секунд", null, async (s, e) =>
                    {
                        MessageBox.Show("Буфер будет очищен через 30 секунд.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void AddAksoDashboard()
        {
            if (AksoWebView != null) return;

            SuspendLayout();

            MaximizeBox = true;
            MinimumSize = new System.Drawing.Size(1180, 720);
            Size = new System.Drawing.Size(Math.Max(1280, Width), Math.Max(780, Height));
            ...
            AksoWebView = new Microsoft.Web.WebView2.WinForms.WebView2()
            {
                Dock = DockStyle.Fill,
                DefaultBackgroundColor = Color.FromArgb(8, 13, 25)
            };

            Controls.Add(AksoWebView);
            AksoWebView.BringToFront();
            PasswordPanel.BringToFront();

            ResumeLayout(true);
            InitializeAksoWebUI();
            StartAksoIntroAnimation();
        }

        private async void InitializeAksoWebUI()
        {
            try
            {
                string uiPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "WebUI", "index.html");

                if (!File.Exists(uiPath))
                {
                    MessageBox.Show($"Не найден WebUI:\n{uiPath}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                await AksoWebView.EnsureCoreWebView2Async();
                AksoWebView.CoreWebView2.Settings.AreDefaultContextMenusEnabled = false;
                AksoWebView.CoreWebView2.Settings.AreDevToolsEnabled = true;
                AksoWebView.CoreWebView2.WebMessageReceived += (s, e) => HandleAksoWebMessage(e.WebMessageAsJson);
                AksoWebView.Source = new Uri(uiPath);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Не удалось запустить новый интерфейс WebUI.\nУстанови Microsoft Edge WebView2 Runtime.\n\n{ex.Message}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void HandleAksoWebMessage(string json)
        {
            try
            {
                JObject message = JObject.Parse(json);
                string action = message["action"]?.Value<string>() ?? string.Empty;
                ...
                switch (action)
                {
                    ...
                    case "selectAccount":
                        SelectAksoAccount(data?["userId"]?.Value<long>() ?? -1);
                        SendAksoWebState();
                        break;

                    case "selectAccounts":
                        SelectAksoAccounts(data?["userIds"] as JArray);
                        SendAksoWebState();
                        break;
                    ...
                    case "removeAccount":
                        Remove_Click(this, EventArgs.Empty);
                        SendAksoWebState();
                        break;

                    case "openBrowser":
                        OpenBrowser_Click(this, EventArgs.Empty);
                        break;
                    ...
                    case "saveServer":
                        SaveToAccount_Click(this, EventArgs.Empty);
                        break;

                    case "loadServers":
                        LoadAksoServers(data?["placeId"]?.Value<string>());
                        break;
                    ...
                    case "updateSetting":
                        if (data != null) SetAksoSetting(data);
                        SendAksoWebState();
                        SendAksoToast("Настройка сохранена");
                        break;
                    ...
                    case "clearClipboard":
                        Clipboard.Clear();
                        SendAksoToast("Буфер обмена очищен");
                        break;

                    default:
                        SendAksoToast("Команда недоступна: " + action);
                        break;
                }
            }
            catch (Exception ex)
            {
                Program.Logger.Error($"WebUI message error: {ex}");
            }
        }

        private void SelectAksoAccounts(JArray userIds)
        {
            SelectedAccounts = new List<Account>();

            if (userIds != null)
            {
                foreach (JToken token in userIds)
                {
            ...
            if (SelectedAccounts.Count > 0)
            {
                ...
                Alias.Text = SelectedAccount.Alias ?? string.Empty;
                DescriptionBox.Text = SelectedAccount.Description ?? string.Empty;
            }
        ...
        private void SelectAksoAccount(long userId)
        {
            AksoSelectedUserId = userId;
            Account account = AccountsList?.FirstOrDefault(x => x.UserID == userId);

            if (account == null) return;

            SelectedAccount = account;
            SelectedAccounts = new List<Account>() { account };
            ...
            Alias.Text = account.Alias ?? string.Empty;
            DescriptionBox.Text = account.Description ?? string.Empty;
        }
        ...
        private void SendAksoWebState()
        {
            if (AksoWebView?.CoreWebView2 == null) return;

            try
            {
                Dictionary<string, object> aksoSettings = new Dictionary<string, object>();

                void AddSetting(IniSection section, string sectionName, string key, string type = "bool")
                {
                    string raw = section != null && section.Exists(key) ? section.Get(key) : "";
                    object value = raw;

                    if (type == "bool")
                        value = string.Equals(raw, "true", StringComparison.OrdinalIgnoreCase);
                    else if (type == "number" && decimal.TryParse(raw, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal number))
                        value = number;

                    aksoSettings[$"{sectionName}.{key}"] = value;
                }

                AddSetting(General, "General", "EnableMultiRbx");
                AddSetting(General, "General", "AsyncJoin");
                AddSetting(General, "General", "AccountJoinDelay", "number");
                AddSetting(General, "General", "SavePasswords");
                AddSetting(General, "General", "AutoCookieRefresh");
                AddSetting(General, "General", "ShowPresence");
                AddSetting(General, "General", "ShuffleJobId");
                AddSetting(General, "General", "ShuffleChoosesLowestServer");
                AddSetting(General, "General", "ShufflePageCount", "number");
                AddSetting(General, "General", "MaxRecentGames", "number");
                AddSetting(General, "General", "ServerRegionFormat", "text");
                AddSetting(General, "General", "UnlockFPS");
                AddSetting(General, "General", "MaxFPSValue", "number");
                AddSetting(General, "General", "AutoCloseLastProcess");
                AddSetting(General, "General", "DisableAgingAlert");
                AddSetting(General, "General", "HideRbxAlert");

                AddSetting(Developer, "Developer", "DevMode");
                AddSetting(Developer, "Developer", "EnableWebServer");

                AddSetting(WebServer, "WebServer", "WebServerPort", "number");
                AddSetting(WebServer, "WebServer", "EveryRequestRequiresPassword");
                AddSetting(WebServer, "WebServer", "Password", "text");
                AddSetting(WebServer, "WebServer", "AllowGetAccounts");
                AddSetting(WebServer, "WebServer", "AllowLaunchAccount");
                AddSetting(WebServer, "WebServer", "AllowAccountEditing");
                AddSetting(WebServer, "WebServer", "AllowGetCookie");
                AddSetting(WebServer, "WebServer", "AllowExternalConnections");

                AddSetting(Watcher, "Watcher", "Enabled");
                AddSetting(Watcher, "Watcher", "ScanInterval", "number");
                AddSetting(Watcher, "Watcher", "ReadInterval", "number");
                AddSetting(Watcher, "Watcher", "ExitOnBeta");
                AddSetting(Watcher, "Watcher", "ExitIfNoConnection");
                AddSetting(Watcher, "Watcher", "NoConnectionTimeout", "number");
                AddSetting(Watcher, "Watcher", "IgnoreExistingProcesses");
                AddSetting(Watcher, "Watcher", "SaveWindowPositions");
                AddSetting(Watcher, "Watcher", "VerifyDataModel");
                AddSetting(Watcher, "Watcher", "CloseRbxMemory");
                AddSetting(Watcher, "Watcher", "MemoryLowValue", "number");
                AddSetting(Watcher, "Watcher", "CloseRbxWindowTitle");
                AddSetting(Watcher, "Watcher", "ExpectedWindowTitle", "text");

                AddSetting(AccountControl, "AccountControl", "StartOnLaunch");
                AddSetting(AccountControl, "AccountControl", "UsePresence");
                AddSetting(AccountControl, "AccountControl", "InternetCheck");
                AddSetting(AccountControl, "AccountControl", "RelaunchDelay", "number");
                AddSetting(AccountControl, "AccountControl", "LauncherDelayNumber", "number");
                AddSetting(AccountControl, "AccountControl", "NexusPort", "number");
                AddSetting(AccountControl, "AccountControl", "AllowExternalConnections");

                var accounts = (AccountsList ?? new List<Account>()).Select(account => new
                {
                    ...
                    group = account.Group,
                    lastUse = account.LastUse == DateTime.MinValue ? "" : account.LastUse.ToString("dd.MM.yyyy HH:mm"),
                    selected = (SelectedAccounts != null && SelectedAccounts.Contains(account)) || account.UserID == AksoSelectedUserId || account == SelectedAccount
                }).ToList();

                int risk = 0;
                if (General?.Get<bool>("SavePasswords") == true) risk++;
                if (Developer?.Get<bool>("DevMode") == true) risk++;
                if (Developer?.Get<bool>("EnableWebServer") == true) risk++;
                if (WebServer?.Get<bool>("AllowGetCookie") == true) risk += 2;
                if (WebServer?.Get<bool>("AllowExternalConnections") == true) risk += 2;
                if (AccountControl?.Get<bool>("AllowExternalConnections") == true) risk += 2;

                string riskLevel = risk == 0 ? "низкий" : risk <= 2 ? "средний" : "высокий";

                object payload = new
                {
                    type = "state",
                    app = RussianLocalization.AppName,
                    accounts = accounts,
                    selectedUserId = AksoSelectedUserId,
                    placeId = PlaceID?.Text ?? string.Empty,
                    jobId = JobID?.Text ?? string.Empty,
                    followUser = UserID?.Text ?? string.Empty,
                    alias = Alias?.Text ?? string.Empty,
                    description = DescriptionBox?.Text ?? string.Empty,
                    currentPlace = CurrentPlace?.Text ?? string.Empty,
                    settings = aksoSettings,
                    stats = new
                    {
                        totalAccounts = accounts.Count,
                        selectedAccounts = SelectedAccounts?.Count ?? 0,
                        riskLevel = riskLevel,
                        webServer = Developer?.Get<bool>("EnableWebServer") == true,
                        nexusExternal = AccountControl?.Get<bool>("AllowExternalConnections") == true
                    }
                };

                AksoWebView.CoreWebView2.PostWebMessageAsJson(JsonConvert.SerializeObject(payload));
            }
            catch (Exception ex)
            {
                Program.Logger.Error($"WebUI state error: {ex}");
            }
        }


        private IniSection GetAksoSettingsSection(string section)
        {
            switch (section)
            {
                ...
                case "AccountControl": return AccountControl;
                case "Watcher": return Watcher;
                default: return null;
            }
        }

        private void SetAksoSetting(string section, string key, bool value)
        {
            IniSection target = GetAksoSettingsSection(section);
        ...
        private async void LoadAksoServers(string rawPlaceId)
        {
            try
            {
                string placeId = Regex.Match(rawPlaceId ?? string.Empty, @"\d+").Value;

                if (string.IsNullOrWhiteSpace(placeId))
                {
                    SendAksoToast("Введите Place ID");
                    return;
                }
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    string json = await client.GetStringAsync($"https://games.roblox.com/v1/games/{placeId}/servers/Public?sortOrder=Asc&limit=100");
                    JObject root = JObject.Parse(json);

                    var servers = root["data"]?.Select(server => new
                    {
                        id = server["id"]?.Value<string>() ?? string.Empty,
                        playing = server["playing"]?.Value<int>() ?? 0,
                        maxPlayers = server["maxPlayers"]?.Value<int>() ?? 0,
                        ping = server["ping"]?.Value<int?>(),
                        fps = server["fps"]?.Value<double?>()
                    }).Where(server => !string.IsNullOrWhiteSpace(server.id)).ToList();
                    ...
                    SendAksoToast($"Серверы загружены: {servers?.Count ?? 0}");
                }
            }
            catch (Exception ex)
            {
                SendAksoToast("Не удалось загрузить серверы");
                Program.Logger.Error($"LoadAksoServers error: {ex}");
            }
        }

        private void SetAksoSetting(JObject data)
        {
            string section = data["section"]?.Value<string>();
            ...
            string value = valueToken?.Type == JTokenType.Boolean
                ? (valueToken.Value<bool>() ? "true" : "false")
                : valueToken?.ToString() ?? string.Empty;
        ...
        private void CreateAccountDataBackup()
        {
            try
            {
                if (!File.Exists(SaveFilePath))
                {
                    MessageBox.Show("Файл AccountData.json ещё не создан.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                Directory.CreateDirectory(Path.Combine(Environment.CurrentDirectory, "Backups"));
                string backupPath = Path.Combine(Environment.CurrentDirectory, "Backups", $"AccountData_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.json");

                SaveAccounts(true, true);
                File.Copy(SaveFilePath, backupPath, true);

                MessageBox.Show($"Backup создан:\n{backupPath}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Не удалось создать backup:\n{ex.Message}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void ExportPublicSettings()
        {
            try
            {
                string exportPath = Path.Combine(Environment.CurrentDirectory, $"akso_public_settings_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.txt");
                StringBuilder sb = new StringBuilder();

                sb.AppendLine("akso alt — экспорт настроек без аккаунтов/cookies");
                sb.AppendLine($"Дата: {DateTime.Now}");
                sb.AppendLine();
                void AppendSection(string title, IniSection section, params string[] hidden)
                {
                    sb.AppendLine($"[{title}]");
                    if (section != null)
                        foreach (var property in section.Properties)
                            sb.AppendLine(hidden.Contains(property.Name) ? $"{property.Name}=<hidden>" : $"{property.Name}={property.Value}");
                    sb.AppendLine();
                }

                AppendSection("General", General);
                AppendSection("Developer", Developer);
                AppendSection("WebServer", WebServer, "Password");
                AppendSection("AccountControl", AccountControl);
                AppendSection("Watcher", Watcher);

                File.WriteAllText(exportPath, sb.ToString(), Encoding.UTF8);
                MessageBox.Show($"Экспорт создан:\n{exportPath}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
        ...
        private void ShowSecurityAudit()
        {
            ...
            Check(General?.Get<bool>("SavePasswords") == true, "Сохранение паролей включено");
            Check(Developer?.Get<bool>("DevMode") == true, "Режим разработчика включён");
            Check(Developer?.Get<bool>("EnableWebServer") == true, "Локальный WebServer включён");
            Check(WebServer?.Get<bool>("AllowGetCookie") == true, "WebServer может отдавать cookies через GetCookie");
            Check(WebServer?.Get<bool>("AllowExternalConnections") == true, "Разрешены внешние подключения к WebServer");
            Check(AccountControl?.Get<bool>("AllowExternalConnections") == true, "Разрешены внешние подключения к Nexus/Account Control");

            string level = risk == 0 ? "низкий" : risk <= 2 ? "средний" : "высокий";
            MessageBox.Show($"Уровень риска: {level}\n\n" + string.Join("\n", lines), "Проверка безопасности — " + RussianLocalization.AppName, MessageBoxButtons.OK, risk >= 3 ? MessageBoxIcon.Warning : MessageBoxIcon.Information);
        }
        ...
        private IEnumerable<Account> GetDisplayedAccounts()
        {
            if (AccountsList == null)
                return Enumerable.Empty<Account>();

            string query = AccountSearchBox?.Text?.Trim();

            if (string.IsNullOrEmpty(query))
                return AccountsList;

            return AccountsList.Where(account =>
                (account.Username ?? string.Empty).IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0 ||
                (account.Alias ?? string.Empty).IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0 ||
                (account.Description ?? string.Empty).IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0 ||
                (account.Group ?? string.Empty).IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0 ||
                account.UserID.ToString().IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0);
        }

        private void UpdateAksoStats()
        {
            if (AccountStatsLabel == null) return;

            int shown = AccountsView.Items.Count;
            int total = AccountsList?.Count ?? 0;
            AccountStatsLabel.Text = string.IsNullOrWhiteSpace(AccountSearchBox?.Text) ? $"{total} аккаунтов" : $"{shown}/{total}";
        }
        ...
        private void RefreshView(object obj = null)
        {
            ...
            {
                AccountsView.SetObjects(GetDisplayedAccounts().ToList());
        ...
        private void LoadAccounts(byte[] Hash = null)
        {
                ...
                if (Header.SequenceEqual(Cryptography.RAMHeader))
                {
                    ...
                    catch (CryptographicException e)
                    {
                        ...
                        catch
                        {
                            ...
                            MessageBox.Show($"Не удалось загрузить аккаунты!\nСоздан backup-файл на случай восстановления данных.\n\n{e.Message}");
                        }
                    }
            }
            ...
            if (!EnteredPassword && AccountsList.Count == 0 && File.Exists($"{SaveFilePath}.backup") && File.ReadAllBytes($"{SaveFilePath}.backup") is byte[] BackupData && BackupData.Length > 0)
            {
                ...
                if (Header.SequenceEqual(Cryptography.RAMHeader) && MessageBox.Show("Найден backup-файл, защищённый паролем. Попробовать загрузить его?", RussianLocalization.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    if (File.Exists(SaveFilePath))
                    {
                ...
                if (MessageBox.Show("Аккаунты не загружены, но найден backup-файл. Загрузить backup?", RussianLocalization.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                {
                    try
                    {
                    ...
                    catch
                    {
                        ...
                        catch { MessageBox.Show("Не удалось загрузить backup-файл!", RussianLocalization.AppName, MessageBoxButtons.OKCancel, MessageBoxIcon.Error); }
                    }
                }
            }
        ...
        private void UnlockButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (PasswordTextBox.Text.Length < 4)
                    Error("Неверный пароль: он должен содержать минимум 4 символа");
            ...
            {
                MessageBox.Show($"Неверный пароль!\n\n{exception.Message}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        private void SetPasswordButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Неверный пароль: он должен содержать минимум 4 символа", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (LastHash.IsEmpty)
            {
                ...
                MessageBox.Show("Подтверди пароль.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
            else
            {
                if (ProtectedData.Unprotect(LastHash.ToArray(), Array.Empty<byte>(), DataProtectionScope.CurrentUser).SequenceEqual(Hash.ToArray()))
                {
                    ...
                    MessageBox.Show("Введён неверный пароль, попробуй ещё раз.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void AccountManager_Load(object sender, EventArgs e)
        {
            ...
            if (false && General.Get<bool>("CheckForUpdates"))
            {
                Task.Run(() =>
                {
                    try
                    {
                        ...
                        if (match.Success)
                        {
                            ...
                            {
                                bool ShouldUpdate = Utilities.YesNoPrompt(RussianLocalization.AppName, "Доступно обновление", "Обновить сейчас?");
            ...
            catch (Exception x) { MessageBox.Show($"Не удалось запустить веб-сервер!\n\n{x}", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error); }
        ...
        private string SendResponse(HttpListenerContext Context)
        {
            ...
            if (AbsolutePath == "/Running") return Reply("akso alt запущен", true, Raw: "true");
            ...
            if (Method == "FollowUser") // https://github.com/ic3w0lf22/Roblox-Account-Manager/pull/52
            {
                ...
                if (!GetUserID(User, out long UserId, out var Response))
                    return Reply($"[{Response.StatusCode} {Response.StatusDescription}] Не удалось получить UserId: {Response.Content}", false);
        ...
        private void AccountManager_Shown(object sender, EventArgs e)
        {
            if (!UpdateMultiRoblox() && !General.Get<bool>("HideRbxAlert"))
                MessageBox.Show("ВНИМАНИЕ: Roblox сейчас запущен. Multi Roblox не заработает, пока ты не перезапустишь менеджер с закрытым Roblox.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            if (AccountControl.Get<bool>("StartOnLaunch"))
                LaunchNexus_Click(this, EventArgs.Empty);
        }
        ...
        private void Remove_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult result = MessageBox.Show($"Точно удалить аккаунты? Количество: {AccountsView.SelectedObjects.Count}", "Удаление аккаунтов", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            ...
            {
                DialogResult result = MessageBox.Show($"Точно удалить аккаунт {SelectedAccount.Username}?", "Удаление аккаунта", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private async void Add_Click(object sender, EventArgs e)
        {
            if (PuppeteerSupported)
            {
                ...
                catch (Exception x)
                {
                    ...
                    if (Utilities.YesNoPrompt("Ошибка при попытке входа", "Возможно, установка Chromium повреждена", "Переустановить Chromium?", false))
                    {
                        MessageBox.Show("akso alt сейчас закроется, потому что нельзя удалить папку, пока она используется.", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void DownloadProgressBar_Click(object sender, EventArgs e)
        {
            ...
            if (TaskDialog.IsPlatformSupported)
            {
                ...
                {
                    Caption = "Добавление аккаунта",
                    InstructionText = $"{(PuppeteerSupported ? "Chromium" : "CefSharp")} ещё загружается",
                    Text = "Если загрузка не работает, можно установить вручную",
                    Icon = TaskDialogStandardIcon.Information
                };

                TaskDialogButton Manual = new TaskDialogButton("Manual", "Установить вручную");
                TaskDialogButton Wait = new TaskDialogButton("Wait", "Подождать");
            ...
            else if (MessageBox.Show($"{(PuppeteerSupported ? "Chromium" : "CefSharp")} ещё загружается. Перед добавлением аккаунта нужно немного подождать.\n\nЕсли загрузка не работает, нажми \"Да\" для ручной установки.", RussianLocalization.AppName, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information) == DialogResult.Yes)
                ShowManualInstallInstructions();
        }

        private void DLChromiumLabel_Click(object sender, EventArgs e) => DownloadProgressBar_Click(sender, e);

        private void manualToolStripMenuItem_Click(object sender, EventArgs e) => Add_Click(this, EventArgs.Empty);

        private void addAccountsToolStripMenuItem_Click(object sender, EventArgs e) => Add_Click(this, EventArgs.Empty);
        ...
        {
            string Combos = ShowDialog("Разделяй аккаунты переносом строки\nФормат: username:password", "Импорт по логин:пароль", big: true);
        ...
        private async void Follow_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"[{Response.StatusCode} {Response.StatusDescription}] Не удалось получить UserId: {Response.Content}");
                return;
            }
        ...
        private void ServerList_Click(object sender, EventArgs e)
        {
            if (AccountsList.Count == 0 || LastValidAccount == null)
                MessageBox.Show("Некоторые функции могут не работать без выбранного валидного аккаунта", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        {
            if (IsDownloadingChromium && !Utilities.YesNoPrompt(RussianLocalization.AppName, $"{(PuppeteerSupported ? "Chromium" : "CefSharp")} ещё загружается. Выход сейчас может повредить установку и помешать работе менеджера", "Всё равно выйти?", false))
            {
                e.Cancel = true;
        ...
        private void BrowserButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Аккаунт не выбран!");
                return;
            }

            CenterAksoForm(UtilsForm);
        }

        private bool ConfirmSensitiveAction(string action, string risk)
        {
            return MessageBox.Show($"{action}\n\n{risk}\n\nПродолжить?", RussianLocalization.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes;
        }

        private void getAuthenticationTicketToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!ConfirmSensitiveAction("Получение Authentication Ticket", "Этот ticket может использоваться для запуска/авторизации сессии. Никому его не отправляй.")) return;
            if (SelectedAccount != null)
            {
        ...
        {
            if (!ConfirmSensitiveAction("Копирование rbx-player ссылки", "Эта ссылка может запускать Roblox от имени выбранного аккаунта. Не отправляй её другим людям.")) return;
            if (SelectedAccount == null) return;
        ...
        {
            if (!ConfirmSensitiveAction("Копирование Security Token / .ROBLOSECURITY", "Этот токен даёт доступ к аккаунту. Никогда не отправляй его другим людям.")) return;
        ...
        {
            if (!ConfirmSensitiveAction("Копирование пароля", "Пароль даёт прямой доступ к аккаунту. Убедись, что рядом никто не увидит буфер обмена.")) return;
        ...
        {
            if (!ConfirmSensitiveAction("Копирование логин:пароль", "Эта связка даёт прямой доступ к аккаунту. Не отправляй её другим людям.")) return;
        ...
        private void moveToToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            string GroupName = ShowDialog("Название группы", "Переместить аккаунт в группу", SelectedAccount != null ? SelectedAccount.Group : string.Empty);
        ...
        private void copyGroupToolStripMenuItem_Click(object sender, EventArgs e) => Clipboard.SetText(SelectedAccount?.Group ?? "Аккаунт не выбран");

        private void copyAppLinkToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!ConfirmSensitiveAction("Копирование app-ссылки", "Эта ссылка содержит данные запуска сессии выбранного аккаунта. Не отправляй её другим людям.")) return;
            if (SelectedAccount == null) return;
        ...
        {
            if (Uri.TryCreate(ShowDialog("URL", "Открыть браузер"), UriKind.Absolute, out Uri Link))
                if (PuppeteerSupported)
                    foreach (Account account in AccountsView.SelectedObjects)
        ...
        private void URLJSToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Uri.TryCreate(ShowDialog("URL", "Открыть браузер"), UriKind.Absolute, out Uri Link))
            {
                string Script = ShowDialog("JavaScript", "Открыть браузер", big: true);
        ...
        {
            if (Uri.TryCreate(ShowDialog("Ссылка группы", "Открыть браузер"), UriKind.Absolute, out Uri Link))
            {
                foreach (Account account in AccountsView.SelectedObjects)
        ...
        private void customURLJSToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (ModifierKeys == Keys.Shift)
                int.TryParse(ShowDialog("Количество (макс. 15)", "Запуск браузера", "1"), out Count);

            if (Uri.TryCreate(ShowDialog("URL", "Запуск браузера", "https://roblox.com/"), UriKind.Absolute, out Uri Link))
            {
                string Script = ShowDialog("JavaScript", "Запуск браузера", big: true);
        ...
        private void SaveToAccount_Click(object sender, EventArgs e)
        {
            if (ModifierKeys == Keys.Shift)
            {
                ...
                if (HasSaved.Count > 0 && MessageBox.Show($"Точно удалить сохранённые PlaceId? Количество: {HasSaved.Count}", RussianLocalization.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.OK)
        ...
        {
            DialogResult result = MessageBox.Show($"Точно отсортировать все аккаунты по алфавиту?", RussianLocalization.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private async void quickLogInToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!Utilities.YesNoPrompt("Быстрый вход", "Вводи только коды, которые запросил сам.\nНикогда не вводи код другого пользователя.", "Понял?", SaveIfNo: false))
                return;
            ...
            string Code = ShowDialog("Код", "Быстрый вход");

            if (Code.Length != 6) { MessageBox.Show("Код Quick Log In должен содержать 6 символов", "Быстрый вход", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        ...
        private void infoToolStripMenuItem1_Click(object sender, EventArgs e) => MessageBox.Show("akso alt", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);

        private void groupsToolStripMenuItem_Click(object sender, EventArgs e) =>
            MessageBox.Show("Группы можно сортировать, добавляя число в начало названия.\nНапример: чтобы группа Apple была сверху, назови её '001 Apple' или '1Apple'.\nЧисла будут скрыты из названия, но сортировка сохранится.\nАккаунты также можно перетаскивать между группами.", RussianLocalization.AppName, MessageBoxButtons.OK, MessageBoxIcon.Information);
